...
export default function Players() {
  ...
  const fetchPlayers = async () => {
    ...
    if (playersError || !playersData) { setLoading(false); return; }

    const { data: teamPlayersData } = await supabase.from('team_players').select('player_id, owner:owners(team_name)');
    const buyerMap: Record<string, string> = {};
    if (teamPlayersData) {
      teamPlayersData.forEach((tp: any) => {
        if (tp.owner?.team_name) buyerMap[tp.player_id] = tp.owner.team_name;
      });
    }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container py-4 sm:py-8 px-4 sm:px-6">
        ...
        <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h1 className="font-display font-bold text-xl sm:text-2xl">Players List</h1>
            <p className="text-sm text-muted-foreground">Browse all players in the auction pool</p>
          </div>
          <PlayerListExport />
        </div>
        ...
        {/* Search and Filters */}
        <Card className="mb-4 sm:mb-6 neu-convex border-0">
          <CardContent className="pt-4 sm:pt-6 pb-4">
            <div className="flex flex-col gap-3">
              <div className="relative flex-1">
                ...
                {searchQuery && (
                  <Button variant="destructive" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 h-7 w-7" onClick={() => setSearchQuery('')}>
                    <X className="w-4 h-4" />
                  </Button>
                )}
              </div>
              ...
              <div key={player.id} className="relative">
                ...
                {player.auction_status === 'sold' && (
                  <div className="absolute top-3 right-3 flex flex-col gap-1 items-end">
                    ...
                    {player.buyer_team && <Badge variant="outline" className="bg-background/80 text-xs">{player.buyer_team}</Badge>}
                  </div>
                )}
      ...
      <Dialog open={!!selectedPlayer} onOpenChange={open => !open && setSelectedPlayer(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto glass-strong border-0 mx-4">
          {selectedPlayer && (
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-3 flex-wrap">
                  <span className="font-display text-xl sm:text-2xl">{selectedPlayer.name}</span>
              ...
              <div className="space-y-4 sm:space-y-6">
                ...
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                  <div className="sm:w-48 shrink-0">
                    {selectedPlayer.profile_picture_url ? (
                      <img src={selectedPlayer.profile_picture_url} alt={selectedPlayer.name} className="w-full h-48 sm:h-56 object-cover rounded-lg" />
                    ) : (
                  ...
                  <div className="flex-1 space-y-4">
                    <div className="grid grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        ...
                        <p className="font-medium text-sm sm:text-base">{selectedPlayer.nationality}</p>
                      </div>
                      <div>
                        <p className="text-xs sm:text-sm text-muted-foreground">Age</p>
                        <p className="font-medium text-sm sm:text-base">{selectedPlayer.age} years</p>
                      </div>
                      <div>
                        <p className="text-xs sm:text-sm text-muted-foreground">Role</p>
                        <p className="font-medium text-sm sm:text-base">{ROLE_LABELS[selectedPlayer.player_role]}</p>
                      </div>
                      <div>
                        <p className="text-xs sm:text-sm text-muted-foreground">Batting Hand</p>
                        <p className="font-medium text-sm sm:text-base capitalize">{selectedPlayer.batting_hand}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div>
                        ...
                        <p className="font-display text-lg sm:text-xl font-bold text-primary">{selectedPlayer.base_price} pts</p>
                      </div>
                ...
                <Card>
                  <CardHeader className="pb-3 px-4 sm:px-6">
                    <CardTitle className="text-sm sm:text-base flex items-center gap-2">
                      <Target className="w-4 h-4" /> Batting Statistics
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="px-4 sm:px-6">
                    <div className="grid grid-cols-3 gap-2 sm:gap-4">
                      <div className="text-center p-2 sm:p-3 bg-muted rounded-lg">
                        <p className="text-lg sm:text-2xl font-bold">{selectedPlayer.total_matches}</p>
                        <p className="text-[10px] sm:text-xs text-muted-foreground">Matches</p>
                      </div>
                      <div className="text-center p-2 sm:p-3 bg-muted rounded-lg">
                        <p className="text-lg sm:text-2xl font-bold">{selectedPlayer.total_runs}</p>
                        <p className="text-[10px] sm:text-xs text-muted-foreground">Runs</p>
                      </div>
                      <div className="text-center p-2 sm:p-3 bg-muted rounded-lg">
                        <p className="text-lg sm:text-2xl font-bold">{selectedPlayer.highest_score}</p>
                        <p className="text-[10px] sm:text-xs text-muted-foreground">Highest</p>
                      </div>
                      <div className="text-center p-2 sm:p-3 bg-muted rounded-lg">
                        <p className="text-lg sm:text-2xl font-bold">{Number(selectedPlayer.strike_rate).toFixed(1)}</p>
                        <p className="text-[10px] sm:text-xs text-muted-foreground">Strike Rate</p>
                      </div>
                      <div className="text-center p-2 sm:p-3 bg-muted rounded-lg">
                        <p className="text-lg sm:text-2xl font-bold text-amber-500">{selectedPlayer.fifties || 0}</p>
                        <p className="text-[10px] sm:text-xs text-muted-foreground">Fifties</p>
                      </div>
                      <div className="text-center p-2 sm:p-3 bg-muted rounded-lg">
                        <p className="text-lg sm:text-2xl font-bold text-green-500">{selectedPlayer.centuries || 0}</p>
                        <p className="text-[10px] sm:text-xs text-muted-foreground">Centuries</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardHeader className="pb-3 px-4 sm:px-6">
                    <CardTitle className="text-sm sm:text-base flex items-center gap-2">
                      <CircleDot className="w-4 h-4" /> Bowling Statistics
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="px-4 sm:px-6">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
                      <div className="text-center p-2 sm:p-3 bg-muted rounded-lg">
                        <p className="text-lg sm:text-2xl font-bold">{selectedPlayer.wickets}</p>
                        <p className="text-[10px] sm:text-xs text-muted-foreground">Wickets</p>
                      </div>
                      <div className="text-center p-2 sm:p-3 bg-muted rounded-lg">
                        <p className="text-lg sm:text-2xl font-bold">{Number(selectedPlayer.bowling_average).toFixed(1)}</p>
                        <p className="text-[10px] sm:text-xs text-muted-foreground">Average</p>
                      </div>
                      <div className="text-center p-2 sm:p-3 bg-muted rounded-lg">
                        <p className="text-lg sm:text-2xl font-bold">{Number(selectedPlayer.economy_rate).toFixed(2)}</p>
                        <p className="text-[10px] sm:text-xs text-muted-foreground">Economy</p>
                      </div>
                      <div className="text-center p-2 sm:p-3 bg-muted rounded-lg">
                        <p className="text-lg sm:text-2xl font-bold">{selectedPlayer.best_bowling || '-'}</p>
                        <p className="text-[10px] sm:text-xs text-muted-foreground">Best Bowling</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
