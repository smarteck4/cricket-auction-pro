...
BEGIN
  ...
  SELECT current_bid, is_active, timer_started_at, timer_duration
    INTO v_current_bid, v_is_active, v_timer_started_at, v_timer_duration
  ...
  -- Check timer hasn't expired (server-side enforcement)
  IF v_timer_started_at IS NOT NULL THEN
    v_elapsed := EXTRACT(EPOCH FROM (NOW() - v_timer_started_at));
    IF v_elapsed >= v_timer_duration THEN
      RETURN jsonb_build_object('error', 'Auction timer has expired');
    END IF;
  END IF;
  ...
  -- Update auction state atomically (resets timer)
